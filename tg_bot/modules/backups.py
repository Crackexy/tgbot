...
import tgbot.modules.sql.notes_sql as sql
...
from tgbot.modules.helper_funcs.alternate import typing_action

# from tgbot.modules.rules import get_rules
import tgbot.modules.sql.rules_sql as rulessql

# from tgbot.modules.sql import warns_sql as warnssql
import tgbot.modules.sql.blacklist_sql as blacklistsql
from tgbot.modules.sql import disable_sql as disabledsql

# from tgbot.modules.sql import cust_filters_sql as filtersql
# import tgbot.modules.sql.welcome_sql as welcsql
import tgbot.modules.sql.locks_sql as locksql
...
def export_data(update, context):
    ...
    f = open("Chiyo{}.backup".format(chat_id), "w")
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open("Chiyo{}.backup".format(chat_id), "rb"),
        caption="*Successfully imported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `Chiyo-Backup` is specially made for notes.".format(
            chat.title, chat_id, tgl
        ),
        timeout=360,
    ...
    os.remove("Chiyo{}.backup".format(chat_id))  # Cleaning file
